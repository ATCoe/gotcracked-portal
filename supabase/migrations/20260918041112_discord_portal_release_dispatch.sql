-- Deliver deployed Portal release notes through the same signed Discord outbox path.
...
begin
  ...
  delivery_url:=case
    when new.entity_type='operator_pin_reset'
      then 'https://uvpmmbioerejeyybfntb.supabase.co/functions/v1/operator-pin-reset-dm'
    else 'https://uvpmmbioerejeyybfntb.supabase.co/functions/v1/discord-outbox-delivery'
  end;
...
revoke all on function public.dispatch_discord_outbox()
