-- GotCracked Portal: premium appointment command center.

insert into public.permission_definitions(permission_key,label,group_name)
values
  ('appointments.view','View appointments','Appointments'),
  ('appointments.manage','Create, schedule, and update appointments','Appointments')
on conflict(permission_key) do update set label=excluded.label,group_name=excluded.group_name;

create or replace function public.role_default_permission(target_role staff_role, permission_key text)
returns boolean
language sql
immutable
...
as $$
  select case
    ...
    when target_role='manager' then permission_key=any(array[
      'dashboard.view','repairs.view','repairs.intake','repairs.workflow','ready_pickup.view','ready_pickup.checkout',
      'leads.view','leads.manage','appointments.view','appointments.manage','customers.view','customers.edit','inventory.view','inventory.manage','inventory.count',
      'purchasing.view','purchasing.manage','reference.view','reference.manage','reports.view','staff.manage','settings.manage',
      'pricing.override','labels.work_order','labels.inventory','schedule.view','schedule.manage','timeclock.use','timeclock.manage'
    ])
    when target_role='technician' then permission_key=any(array[
      'dashboard.view','repairs.view','repairs.intake','repairs.workflow','ready_pickup.view','leads.view','leads.manage','appointments.view',
      'customers.view','inventory.view','reference.view','labels.work_order','schedule.view','timeclock.use'
    ])
    when target_role='front_desk' then permission_key=any(array[
      'dashboard.view','repairs.view','repairs.intake','ready_pickup.view','ready_pickup.checkout','leads.view','leads.manage','appointments.view','appointments.manage',
      'customers.view','customers.edit','inventory.view','reference.view','labels.work_order','schedule.view','timeclock.use'
    ])
    else false
  end
$$;
...
alter table public.appointments
  add constraint appointments_duration_minutes_check check(duration_minutes between 15 and 480);

create index if not exists appointments_location_start_idx on public.appointments(location_id,starts_at);
create index if not exists appointments_location_status_start_idx on public.appointments(location_id,status,starts_at);
create index if not exists appointments_assigned_start_idx on public.appointments(assigned_user_id,starts_at) where assigned_user_id is not null;

alter table public.appointments enable row level security;
drop policy if exists "staff can view location appointments" on public.appointments;
drop policy if exists "staff can add location appointments" on public.appointments;
drop policy if exists "staff can update location appointments" on public.appointments;
create policy "permissioned staff can view appointments"
on public.appointments for select to authenticated
using(location_id=(select public.current_location_id()) and (select coalesce(public.has_permission('appointments.view'),false)));
create policy "permissioned staff can add appointments"
on public.appointments for insert to authenticated
with check(location_id=(select public.current_location_id()) and (select coalesce(public.has_permission('appointments.manage'),false)));
create policy "permissioned staff can update appointments"
on public.appointments for update to authenticated
using(location_id=(select public.current_location_id()) and (select coalesce(public.has_permission('appointments.manage'),false)))
with check(location_id=(select public.current_location_id()) and (select coalesce(public.has_permission('appointments.manage'),false)));
  ...
  select jsonb_build_object(
    ...
    'appointments',coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'customer_name',c.full_name,'customer_phone',c.phone,'customer_email',c.email
      ...
      where a.location_id=loc
        and coalesce((a.starts_at at time zone 'America/New_York')::date,a.preferred_date,a.created_at::date) between range_start and range_end
    ),'[]'::jsonb),
    ...
    'conflicts',case when can_manage then coalesce((
      ...
      where a.location_id=loc and a.assigned_user_id is not null
        and (a.starts_at at time zone 'America/New_York')::date between range_start and range_end
    ),'[]'::jsonb) else '[]'::jsonb end
  ) into result;
  return result;
end;
...
begin
  ...
  values(loc,btrim(customer_name_input),nullif(btrim(coalesce(phone_input,'')),''),nullif(lower(btrim(coalesce(email_input,''))),''),btrim(service_input),'portal-appointment',nullif(btrim(coalesce(notes_input,'')),''),'qualified','qualified',nullif(btrim(coalesce(device_input,'')),''),btrim(service_input),(starts_at_input at time zone 'America/New_York')::date,starts_at_input)
  ...
  values(loc,lead_row.id,nullif(btrim(coalesce(device_input,'')),''),btrim(service_input),starts_at_input,(starts_at_input at time zone 'America/New_York')::date,'confirmed',nullif(btrim(coalesce(notes_input,'')),''),'walk_in',coalesce(duration_input,60),assigned_user_input,'portal',now())
...
set search_path=public
as $$
declare saved public.appointments; loc uuid:=public.current_location_id();
begin
  ...
  update public.leads set expected_arrival_at=starts_at_input,preferred_date=(starts_at_input at time zone 'America/New_York')::date,updated_at=now() where id=saved.lead_id;
  return saved;
end;
$$;
...
begin
  if auth.uid() is null or loc is null or not coalesce(public.has_permission('appointments.manage'),false) then raise exception 'Appointment management permission is required.'; end if;
  if status_input not in ('requested','scheduled','confirmed','arrived','no_show','cancelled','completed') then raise exception 'Invalid appointment status.'; end if;
  update public.appointments set
    status=status_input,
    confirmed_at=case when status_input in ('scheduled','confirmed') then coalesce(confirmed_at,now()) else confirmed_at end,
    arrived_at=case when status_input='arrived' then coalesce(arrived_at,now()) else arrived_at end,
    completed_at=case when status_input='completed' then coalesce(completed_at,now()) else completed_at end,
    cancelled_at=case when status_input='cancelled' then coalesce(cancelled_at,now()) else cancelled_at end,
    notes=case when nullif(btrim(coalesce(note_input,'')),'') is null then notes else concat_ws(E'\n',notes,btrim(note_input)) end,
    updated_at=now()
  where id=appointment_id_input and location_id=loc
  returning * into saved;
  if saved.id is null then raise exception 'Appointment not found.'; end if;
  return saved;
end;
$$;
