...
begin
  ...
  with rates as (
    ...
    where sc.location_id=loc and p.active=true and p.role::text in ('technician','manager')
      and sc.effective_at<=public.current_business_date(loc)
  )
  select avg(hourly_cents),count(hourly_cents) into basis,rate_count
  from rates where hourly_cents>0;
...
declare
  ...
  part public.inventory_items;  bs public.business_settings;
...
begin
  select t.location_id,t.assigned_user_id,d.category into loc,assigned_employee,device_category
  ...
  select * into part from public.inventory_items where id=target_inventory_item and location_id=loc and active;
  ...
  elsif guide_minutes is not null then minutes:=guide_minutes;  else
  ...
  if assigned_employee is not null then
    ...
    into labor_cost_hourly_cents
    from public.staff_compensation sc join public.profiles p on p.id=sc.profile_id
    where sc.profile_id=assigned_employee and sc.location_id=loc and p.active=true
      and sc.effective_at<=public.current_business_date(loc)
    limit 1;
    if labor_cost_hourly_cents is not null and labor_cost_hourly_cents>0 then
      labor_cost_source:='assigned_employee_compensation';
    end if;
  end if;

  if labor_cost_hourly_cents is null or labor_cost_hourly_cents<=0 then
    select avg(rate) into labor_cost_hourly_cents from (
      select case
        when sc.employment_type='hourly' and sc.hourly_rate_cents>0 then sc.hourly_rate_cents::numeric
        when sc.employment_type='salary' and sc.weekly_salary_cents>0 then sc.weekly_salary_cents::numeric/40.0
        else null end rate
      from public.staff_compensation sc join public.profiles p on p.id=sc.profile_id and p.location_id=sc.location_id      where sc.location_id=loc and p.active=true and p.role::text in ('technician','manager')
        and sc.effective_at<=public.current_business_date(loc)
    ) r where rate>0;
    if labor_cost_hourly_cents is not null and labor_cost_hourly_cents>0 then
      labor_cost_source:='blended_employee_compensation';
    end if;
  end if;
  ...
  return jsonb_build_object(
    ...
    'labor_cost_source',labor_cost_source,    'estimated_labor_cost_cents',estimated_labor_cost_cents,
