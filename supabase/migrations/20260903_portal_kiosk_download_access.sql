-- The tablet kiosk installer is a private operational bundle. Keep its
-- decision server-side and align it with the Portal Companion's human-session
-- rules so a disabled staff account cannot keep downloading it.
create or replace function public.portal_kiosk_download_access_status()
returns jsonb
language plpgsql
stable
security definer
...
revoke all on function public.portal_kiosk_download_access_status() from public, anon;
grant execute on function public.portal_kiosk_download_access_status() to authenticated;