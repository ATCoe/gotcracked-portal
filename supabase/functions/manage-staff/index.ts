...
Deno.serve(async request => {
  ...
  try {
    ...
    if (permission.error || permission.data !== true) return json(request, { error:'Staff management permission is required.' }, 403);
    ...
    if (targetUserId === user.id && body.active === false) return json(request, { error: 'You cannot deactivate your own account.' }, 400);

    const { data: target } = await admin.from('profiles')
      .select('id,location_id,display_name,role,active,discord_user_id,account_type,job_title')
      .eq('id', targetUserId)
      .single();
    if (!target || target.location_id !== actor.location_id) return json(request, { error: 'Staff member not found for this location.' }, 404);
    if (target.account_type === 'shared_workstation') {
      return json(request, { error: 'Shared workstations are managed from Settings → Trusted shop workstations, not Employee access.' }, 403);
    }
    ...
    if (actor.role === 'manager' && target.role === 'manager') return json(request, { error: 'Managers cannot manage other managers.' }, 403);
