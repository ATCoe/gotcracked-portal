...
Deno.serve(async request=>{
  ...
  try{
    ...
    const session=await userClient.rpc('portal_session_authorized');
    if(session.error||session.data!==true)return respond(origin,{error:'A verified Portal staff session is required.'},403);
    ...
    const portalEmail=clean(body.portalEmail,160).toLowerCase();
    ...
    if(!/^[a-z0-9._%+-]+@gotcracked\.co$/.test(portalEmail))return respond(origin,{error:'Enter the employee’s active @gotcracked.co Google Workspace address.'},400);
    ...
    const profileEmail=await admin.from('profiles').select('id').ilike('portal_email',portalEmail).maybeSingle();
    if(profileEmail.data)return respond(origin,{error:'That Workspace address is already assigned to a Portal profile.'},409);
    const pendingEmail=await admin.from('staff_invitations').select('id').eq('location_id',actor.location_id).ilike('portal_email',portalEmail).is('used_at',null).is('cancelled_at',null).gt('expires_at',new Date().toISOString()).maybeSingle();
    if(pendingEmail.data)return respond(origin,{error:'An active onboarding package already exists for this Workspace address. Cancel it before issuing another.'},409);
    ...
    const welcomeEmailBody=`Hi ${displayName.split(' ')[0]},\n\n${welcomeMessage}\n${titleLine}${firstDayLine}\n\nYour secure onboarding steps:\n1. Sign in to your Google Workspace account: ${portalEmail}\n2. Open your private Portal invitation: ${portalInviteUrl}\n3. Choose Continue with Google Workspace and complete the guided welcome checklist.\n4. Join the GotCracked staff Discord for your fallback sign-in and team access: ${discordInviteUrl}\n5. Create your personal workstation PIN when prompted.\n\nGoogle Workspace is your primary Portal identity. Discord remains your linked fallback. No shared or temporary Portal password is issued.\n\nIf you have questions before your first day, reply to this email.\n\n— GotCracked Management`;
