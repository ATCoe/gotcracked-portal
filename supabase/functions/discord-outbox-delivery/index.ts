...
function supportPayload(row:any){
  ...
  return {
    ...
    embeds:[{
      ...
      color:p.status==='resolved'||p.status==='closed'?0x2fbf71:p.priority==='critical'?0xe5484d:p.priority==='high'?0xf59e0b:0x159bd3,
      fields,
      footer:{text:'GotCracked Tech Support · silent log'},
      timestamp:new Date(row.created_at).toISOString()
    }],
...
Deno.serve(async request=>{
  ...
  try{
    ...
    return new Response(JSON.stringify({ok:true,silent:row.entity_type==='support_ticket'}),{headers:{'Content-Type':'application/json'}});
  }catch(error){
    console.error(error);
