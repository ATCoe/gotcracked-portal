(() => {
  ...
  function injectStyle(){
    ...
    style.textContent=`
      ...
      #customers .gc-customer-columns{font-size:.72rem;text-transform:uppercase;letter-spacing:.05em;color:#647181;font-weight:700;cursor:default;background:rgba(100,116,139,.04)}
  ...
  function renderDirectory(target){
    ...
    target.innerHTML=`<div class="gc-customer-shell">
      <div class="gc-customer-header"><div><p class="eyebrow">Global database</p><h1>Customer command center</h1><p class="subtle">Search the shared customer, device, repair, and contact directory from one place. Repair activity remains scoped to your location.</p></div><div class="gc-customer-actions">${data.can_edit&&!training()?'<button class="primary-button" type="button" data-customer-new>+ New customer</button>':''}<button class="secondary-button" type="button" data-customer-refresh>Refresh</button></div></div>
  ...
  function openCustomerForm(customer=null){
    ...
    dialog.innerHTML=`<form id="gc-customer-form" class="gc-customer-form"><div class="gc-customer-dialog-head"><div><p class="eyebrow">Customer profile</p><h2>${c.id?'Edit customer':'New customer'}</h2><p class="subtle">Keep contact information current so every repair stays connected.</p></div><button class="icon-button" type="button" data-customer-dialog-close aria-label="Close customer form">×</button></div><div class="gc-customer-form-grid"><label>First name<input name="first_name" required value="${esc(c.first_name||'')}"></label><label>Last name<input name="last_name" value="${esc(c.last_name||'')}"></label><label>Primary phone<input name="phone" type="tel" required value="${esc(c.phone||'')}"></label><label>Alternate phone<input name="contact_phone" type="tel" value="${esc(c.contact_phone&&c.contact_phone!==c.phone?c.contact_phone:'')}"></label><label>Email<input name="email" type="email" value="${esc(c.email||'')}"></label><label>Preferred contact<select name="preferred_contact"><option value="sms" ${c.preferred_contact==='sms'?'selected':''}>Text / SMS</option><option value="call" ${c.preferred_contact==='call'?'selected':''}>Phone call</option><option value="email" ${c.preferred_contact==='email'?'selected':''}>Email</option></select></label><label class="full">Address<input name="address_line_1" value="${esc(c.address_line_1||'')}"></label><label class="full">Address line 2<input name="address_line_2" value="${esc(c.address_line_2||'')}"></label><label>City<input name="city" value="${esc(c.city||'')}"></label><label>State<input name="state" maxlength="2" value="${esc(c.state||'')}"></label><label>ZIP<input name="postal_code" value="${esc(c.postal_code||'')}"></label><label class="full">Customer notes<textarea name="notes" rows="4">${esc(c.notes||'')}</textarea></label></div><input type="hidden" name="customer_id" value="${esc(c.id||'')}"><div class="gc-customer-dialog-actions"><span class="gc-customer-form-message" role="status"></span><button class="secondary-button" type="button" data-customer-dialog-close>Cancel</button><button class="primary-button" type="submit">${c.id?'Save customer':'Create customer'}</button></div></form>`;
    dialog.showModal();
  }
