(() => {
  ...
  function ensureButton() {
    ...
    if (!actions) return;
    const existing = actions.querySelector('[data-open-inventory-audit]');
    if (!canCount()) { existing?.remove(); return; }
    if (!existing) actions.insertAdjacentHTML('beforeend','<button class="secondary-button" data-open-inventory-audit>Inventory audit</button>');
  }
  ...
  function render() {
    if (!audit) {
      host.innerHTML = `<div class="modal-head"><div><p class="eyebrow">Inventory control</p><h2>Start a barcode audit</h2></div><button class="icon-button" type="button" data-close-audit aria-label="Close">×</button></div><div class="audit-intro"><span>▤</span><h3>Snapshot every active inventory item.</h3><p>Scan each physical part label. Repeated scans count multiple units. When a permitted staff member completes the audit, Portal adjusts stock to the counted quantities and logs every variance.</p></div>${canCount() ? '<button class="primary-button" data-start-audit>Start inventory audit</button>' : '<p class="auth-message">Inventory Count permission is required to start or work an audit.</p>'}<p class="audit-status auth-message"></p>`;
      return;
    }
