(()=>{
  ...
  const capacityIntent=text=>/\b(queue|workload|capacity|appointment|booking|booked|same[- ]?day|turnaround|repair time|repair timing|how long|throughput|repair in progress|parts delay|parts availability|bench time|active repair|repair average|average repair)\b/i.test(text);
  ...
  async function contextFor(text){
    ...
    if(capacityIntent(text)){
      ...
      out.capacityPolicy='Measure repair labor only while status is repair_in_progress; every departure pauses the active timer and a later return resumes with a new interval. Prefer model+service timing after at least 3 matching jobs, then model timing after 3, then device-class timing after 5, otherwise use conservative defaults. Phones are the only category eligible for a same-day promise, and only when required parts and conservative published-staff capacity both permit it. Treat every other device class as multi-day. Never promise same-day when no technician schedule is published.';
    }
    return out;
  }
