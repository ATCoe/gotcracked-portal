...
// All identities, sessions and Discord responses below are synthetic. No network
// or production database is used, and rejected cases must make zero writes.
...
function harness(options = {}) {
  let handler, writes=0, memberRequests=0;
  ...
  const client={
    ...
    from:table=>{
      assert.ok(['profiles','portal_human_sessions'].includes(table),'Unexpected table access');
  ...
  const context={Deno:{env:{get:()=> 'synthetic-configuration'},serve:fn=>handler=fn},createClient:()=>client,Request,Response,Headers,URL,TextEncoder,crypto,atob,console:{error(){}},fetch:async url=>{
    assert.match(url,/^https:\/\/discord\.com\/api\/v10\/guilds\//);memberRequests++;
  ...
  const request=new Request('https://example.invalid/discord-verify',{method:options.httpMethod||'POST',headers:{Origin:options.origin||'https://portal.gotcracked.co',Authorization:'Bearer '+token,'Content-Type':'application/json'},...(options.httpMethod==='GET'?{}:{body:'{}'})});
  return {run:()=>handler(request),writes:()=>writes,members:()=>memberRequests};
}
let passed=0;
async function test(name,options,status,{members=0,writes=0}={}){
  ...
  assert.equal(h.members(),members,name+': unexpected Discord request');
...
await test('non-member cannot register a Portal session',{notMember:true},403,{members:1});
await test('inactive profile rejected',{staffActive:false},403,{members:1});
await test('shared workstation cannot become human Discord staff',{accountType:'shared_workstation'},403,{members:1});
await test('automation cannot become human Discord staff',{accountType:'automation'},403,{members:1});
await test('mismatched linked profile rejected',{mismatch:true},403,{members:1});
await test('valid current Discord staff session accepted',{},200,{members:1,writes:2});
