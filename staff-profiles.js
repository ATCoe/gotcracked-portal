(() => {
  ...
  async function load(){
    ...
    const {data,error}=await client.from('profiles').select('id,location_id,display_name,role,active,avatar_url,job_title,phone,bio,discord_user_id,discord_username,recovery_email,created_at,updated_at').eq('location_id',p.location_id).order('display_name');
