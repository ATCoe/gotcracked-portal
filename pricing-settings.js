(() => {
  ...
  function trainingState(){
    ...
    return raw || {
      settings:{target_gross_margin_percent:50,target_splh:125,target_labor_percent:.18,charge_parts_to_customer:false,auto_service_taxable:true},
  ...
  async function load(){
    ...
    const [business,parts,refs] = await Promise.all([
      client.from('business_settings').select('location_id,target_gross_margin_percent,target_splh,target_labor_percent,charge_parts_to_customer,auto_service_taxable').eq('location_id',profile.location_id).maybeSingle(),
