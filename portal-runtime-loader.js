(() => {
  ...
  async function loadSequence(files) { for(const file of files){if(file!=='theme-controller.js'&&file!=='training-shared-sync.js'&&file!=='operations-v1-core.js')await waitForOperationsProfile();await loadScript(file);if(file==='training-shared-sync.js'&&window.GotCrackedTrainingSync?.ready)await window.GotCrackedTrainingSync.ready;if(file==='operations-v1-core.js')await waitForOperationsProfile();} }
  async function startCriticalRuntime(){if(started)return;started=true;document.documentElement.dataset.gcRuntimeState='starting';document.documentElement.dataset.gcPortalBoot='loading';preload(criticalScripts);try{await loadSequence(criticalScripts);const currentView=location.hash.slice(1).split('/')[0]||'dashboard';await ensureViewRuntime(currentView);document.documentElement.dataset.gcRuntimeState='ready';document.documentElement.dataset.gcPortalBoot='ready';document.dispatchEvent(new CustomEvent('gc-portal-runtime-ready',{detail:{profile:profileReady}}));scheduleDeferredRuntime();}catch(error){console.error('Portal critical runtime load failed:',error);document.documentElement.dataset.gcRuntimeState='error';document.documentElement.dataset.gcPortalBoot='error';window.GotCrackedDiagnostics?.error?.(error,{context:'Portal staff profile initialization failed',duration:20000});started=false;}}
  async function startDeferredRuntime(){if(deferredStarted)return;deferredStarted=true;try{await waitForOperationsProfile();await loadSequence(deferredScripts);document.dispatchEvent(new CustomEvent('gc-portal-secondary-runtime-ready'));}catch(error){console.error('Portal deferred runtime load failed:',error);deferredStarted=false;}}
  ...
  async function ensureViewRuntime(view){const files=viewDependencies[view]||[];if(!files.length)return;try{await waitForOperationsProfile();await loadSequence(files);}catch(error){console.error(`Portal ${view} runtime load failed:`,error);window.GotCrackedDiagnostics?.error?.(error,{context:`Unable to open ${view}`});}}
