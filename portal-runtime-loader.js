(() => {
  ...
  const VERSION = '20260826-release23';
  ...
  function preload(files) {
    for (const file of files) {
      ...
      link.rel = 'preload'; link.as = 'script'; link.href = srcFor(file); link.dataset.gcRuntimePreload = file;
  ...
  async function waitForOperationsProfile() {
    ...
    while (Date.now() - startedAt < PROFILE_READY_TIMEOUT_MS) {
      ...
      if (profile?.id && profile?.location_id && profile.active !== false) {
        profileReady = profile; window.GotCrackedRuntimeProfile = profile; document.documentElement.dataset.gcProfileReady = 'true'; return profile;
      }
      await delay(100);
    }
  ...
  async function startCriticalRuntime() {
    if (started) return;
    started = true; document.documentElement.dataset.gcRuntimeState = 'starting'; document.documentElement.dataset.gcPortalBoot = 'loading'; preload(criticalScripts);
    try {
      ...
      document.documentElement.dataset.gcRuntimeState = 'ready'; document.documentElement.dataset.gcPortalBoot = 'ready';
      document.dispatchEvent(new CustomEvent('gc-portal-runtime-ready', { detail:{ profile:profileReady } }));
      scheduleDeferredRuntime();
    } catch (error) {
      console.error('Portal critical runtime load failed:', error); document.documentElement.dataset.gcRuntimeState = 'error'; document.documentElement.dataset.gcPortalBoot = 'error';
      window.GotCrackedDiagnostics?.error?.(error, { context:'Portal staff profile initialization failed', duration:20000 }); started = false;
    }
  }

  async function startDeferredRuntime() {
    if (deferredStarted) return; deferredStarted = true;
    try { await waitForOperationsProfile(); await loadSequence(deferredScripts); document.dispatchEvent(new CustomEvent('gc-portal-secondary-runtime-ready')); }
    catch (error) { console.error('Portal deferred runtime load failed:', error); deferredStarted = false; }
  }

  function scheduleDeferredRuntime() { const run = () => startDeferredRuntime(); if ('requestIdleCallback' in window) window.requestIdleCallback(run, { timeout: 2600 }); else setTimeout(run, 1600); }

  async function ensureViewRuntime(view) {
    const files = viewDependencies[view] || []; if (!files.length) return;
    try { await waitForOperationsProfile(); await loadSequence(files); }
    catch (error) { console.error(`Portal ${view} runtime load failed:`, error); window.GotCrackedDiagnostics?.error?.(error, { context:`Unable to open ${view}` }); }
  }

  function scheduleCriticalStart() { const run = () => startCriticalRuntime(); requestAnimationFrame(() => requestAnimationFrame(run)); }

  function watchLoginState() {
    const login = document.getElementById('login-screen'); if (!login) return;
    if (login.classList.contains('hidden')) { scheduleCriticalStart(); return; }
    const observer = new MutationObserver(() => { if (!login.classList.contains('hidden')) return; observer.disconnect(); scheduleCriticalStart(); });
    observer.observe(login, { attributes: true, attributeFilter: ['class'] });
  }

  document.addEventListener('click', event => { const target = event.target instanceof Element ? event.target.closest('[data-view]') : null; const view = target?.dataset.view; if (view) ensureViewRuntime(view); }, true);
