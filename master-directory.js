(() => {
  ...
  const VERSION = '20260828-directory5';
  ...
  async function load() {
    ...
    try {
      ...
      let [repairs,leads]=await fetchDirectoryData();
      if (isAuthFailure(repairs)||isAuthFailure(leads)) { const refreshed=await client.auth.refreshSession(); if (refreshed.error||!refreshed.data?.session) throw refreshed.error||repairs.error||leads.error; [repairs,leads]=await fetchDirectoryData(); }
