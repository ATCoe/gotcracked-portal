(() => {
  ...
  const VERSION = '20260902-directory-global1';
  ...
  const state = { dashboard:baseState(), leads:baseState(), data:{repairs:[],leads:[],customers:[],devices:[],parts:[]}, loading:false, loaded:false, refreshTimer:null };
  ...
  function normalizeLead(lead) {
    ...
    const device = [lead.manufacturer,lead.model].filter(Boolean).join(' ').trim() || lead.device_category || lead.service || 'Device not identified';
    const service = lead.customer_issue || lead.service || lead.notes || 'Service inquiry';
    return { key:`lead:${lead.id}`, type:'lead', sourceId:lead.id, ticketNumber:'', displayId:`LD-${String(lead.id || '').replaceAll('-','').slice(0,6).toUpperCase() || 'NEW'}`, customer:lead.name || 'Unknown lead', phone:lead.phone || '', email:lead.email || '', device, deviceType:lead.device_category || 'Lead / inquiry', service, status:stage, statusLabel:labelFor('lead',stage), assigned:lead.profiles?.display_name || 'Unassigned', intake:lead.intake_method || lead.source || 'lead', createdAt:lead.created_at || lead.updated_at || '', updatedAt:lead.updated_at || lead.created_at || '' };
  }
  function normalizeCustomer(customer) {
    const devices=Array.isArray(customer.devices)?customer.devices:[];
    return { key:`customer:${customer.id}`, type:'customer', sourceId:customer.id, ticketNumber:'', displayId:`CUS-${String(customer.id||'').replaceAll('-','').slice(0,6).toUpperCase()||'NEW'}`, customer:[customer.first_name,customer.last_name].filter(Boolean).join(' ')||'Customer', phone:customer.phone||'', email:customer.email||'', device:`${devices.length} saved device${devices.length===1?'':'s'}`, deviceType:'Customer', service:'Global customer record', status:'global', statusLabel:'Global customer', assigned:'Shared database', intake:'global', createdAt:customer.created_at||'', updatedAt:customer.updated_at||customer.created_at||'' };
  }
  function normalizeDevice(device) {
    const customer=device.customers||{};
    const label=[device.manufacturer,device.model].filter(Boolean).join(' ').trim()||device.category||'Device';
    return { key:`device:${device.id}`, type:'device', sourceId:device.id, ticketNumber:'', displayId:`DEV-${String(device.id||'').replaceAll('-','').slice(0,6).toUpperCase()||'NEW'}`, customer:[customer.first_name,customer.last_name].filter(Boolean).join(' ')||'Unlinked customer', phone:customer.phone||'', email:customer.email||'', device:label, deviceType:device.category||'Device', service:[device.serial_number&&`Serial ${device.serial_number}`,device.imei&&`IMEI ${device.imei}`].filter(Boolean).join(' · ')||'Saved device', status:'global', statusLabel:'Global device', assigned:'Shared database', intake:'global', createdAt:device.created_at||'', updatedAt:device.updated_at||device.created_at||'' };
  }
  function normalizePart(part) {
    const quantity=Number(part.quantity_on_hand||0), reorder=Number(part.reorder_point||0);
    const status=!part.active?'inactive':quantity<=0?'out_of_stock':quantity<=reorder?'low_stock':'in_stock';
    return { key:`part:${part.id}`, type:'part', sourceId:part.id, ticketNumber:'', displayId:part.sku||`PART-${String(part.id||'').replaceAll('-','').slice(0,6).toUpperCase()||'NEW'}`, customer:'Global parts catalog', phone:'', email:'', device:part.name||'Part', deviceType:part.category||'Part', service:`${quantity} on hand · reorder at ${reorder}`, status, statusLabel:status==='in_stock'?'In stock':status==='low_stock'?'Low stock':status==='out_of_stock'?'Out of stock':'Inactive', assigned:'Shared inventory', intake:'global', createdAt:part.created_at||'', updatedAt:part.updated_at||part.created_at||'' };
  }
  function allRecords(scope) { return scope === 'leads' ? state.data.leads.map(normalizeLead) : [...state.data.repairs.map(normalizeRepair),...state.data.leads.map(normalizeLead),...state.data.customers.map(normalizeCustomer),...state.data.devices.map(normalizeDevice),...state.data.parts.map(normalizePart)]; }
  ...
  const typeLabel = type => ({work_order:'Work order',lead:'Lead',customer:'Customer',device:'Device',part:'Part'}[type]||friendly(type));
  function statusClass(record) { const v=record.status; if (['need_to_contact','new','need_to_order_part','need_to_order_parts','awaiting_repair','awaiting_diagnostic','checked_in','out_of_stock'].includes(v)) return 'status-red'; if (['awaiting_customer','awaiting_device','claimed','awaiting_approval','awaiting_callback','awaiting_parts','waiting_on_parts','low_stock'].includes(v)||String(v).startsWith('awaiting_')) return 'status-yellow'; if (['repaired','ready_for_pickup','testing_in_progress','repair_in_progress','converted','won','customer_declined','unrepairable','in_stock','global'].includes(v)) return 'status-green'; if (v==='sale_complete'||v==='inactive') return 'status-blue'; return ''; }
  ...
  function drawResults(scope) {
    ...
    const filtered=filteredRecords(scope), base=allRecords(scope).filter(record=>state[scope].includeClosed||!isClosed(record)), activeCount=base.filter(record=>!isClosed(record)).length, workOrderCount=filtered.filter(r=>r.type==='work_order').length, leadCount=filtered.filter(r=>r.type==='lead').length, globalCount=filtered.filter(r=>['customer','device','part'].includes(r.type)).length;
    summary.innerHTML=`<strong>${filtered.length}</strong> of ${base.length} records shown${scope==='dashboard'?` · ${workOrderCount} work order${workOrderCount===1?'':'s'} · ${leadCount} lead${leadCount===1?'':'s'} · ${globalCount} shared record${globalCount===1?'':'s'}`:` · ${activeCount} active`}`;
  ...
  function ensureHost(scope) {
    if (scope==='dashboard') {
      ...
      dashboard.classList.add('gc-directory-mounted'); const subtitle=dashboard.querySelector('.page-heading .subtle'); if (subtitle) subtitle.textContent='Search and work shared customers, devices, parts, repairs, and leads from one master directory.'; markDashboardMetrics(); return host;
    }
  ...
  function setPreset(scope,preset) {
    ...
    if (scope==='dashboard') { s.includeClosed=false; if (preset==='attention') ['lead:need_to_contact','work_order:need_to_order_parts','work_order:awaiting_parts','work_order:awaiting_callback','work_order:awaiting_approval'].forEach(v=>s.statuses.add(v)); if (preset==='new_leads') {s.types.add('lead');s.statuses.add('lead:need_to_contact');} if (preset==='awaiting_parts') {s.types.add('work_order');['work_order:need_to_order_parts','work_order:awaiting_parts','work_order:waiting_on_parts'].forEach(v=>s.statuses.add(v));} if (preset==='ready') {s.types.add('work_order');['work_order:repaired','work_order:ready_for_pickup'].forEach(v=>s.statuses.add(v));} if (preset==='unassigned') s.assigned='Unassigned'; }
    else { s.includeClosed=['converted','lost'].includes(preset); if (['need_to_contact','awaiting_customer','awaiting_device','need_to_order_part','awaiting_parts','converted','lost'].includes(preset)) s.statuses.add(`lead:${preset}`); if (preset==='unassigned') s.assigned='Unassigned'; }
  ...
  const fetchDirectoryData = () => Promise.all([client.from('repair_tickets').select('id,ticket_number,status,created_at,updated_at,intake_method,customer_issue,customers(first_name,last_name,phone,email),devices(model,manufacturer,category),profiles:assigned_user_id(display_name)').order('created_at',{ascending:false}).limit(500),client.from('leads').select('id,name,phone,email,service,notes,source,status,pipeline_status,device_category,manufacturer,model,customer_issue,contact_attempted_at,created_at,updated_at,intake_method,assigned_user_id,profiles:assigned_user_id(display_name)').order('created_at',{ascending:false}).limit(500),client.from('customers').select('id,first_name,last_name,phone,email,created_at,devices(id,manufacturer,model,category,serial_number,imei,created_at)').order('created_at',{ascending:false}).limit(1000),client.from('devices').select('id,customer_id,category,manufacturer,model,serial_number,imei,created_at,customers(first_name,last_name,phone,email)').order('created_at',{ascending:false}).limit(2000),client.from('inventory_items').select('id,sku,name,quantity_on_hand,reorder_point,active').eq('active',true).order('name').limit(5000)]);
  const isAuthFailure = result => Number(result?.status||result?.error?.status||0)===401 || /(?:jwt|token|unauthorized)/i.test(String(result?.error?.message||''));
