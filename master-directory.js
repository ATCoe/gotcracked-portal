(() => {
  ...
  function statusClass(record) { const v=record.status; if (['need_to_contact','new','awaiting_repair','checked_in'].includes(v)) return 'new'; if (['awaiting_customer','awaiting_device','claimed','awaiting_approval','awaiting_callback'].includes(v)) return 'callback'; if (['qualified','diagnostic_in_progress','in_diagnosis'].includes(v)) return 'diagnosis'; if (['need_to_order_part','need_to_order_parts','awaiting_parts','waiting_on_parts'].includes(v)) return 'parts'; if (['repair_in_progress','in_repair'].includes(v)) return 'in-repair'; if (v==='quality_inspection') return 'inspection'; if (['repaired','ready_for_pickup','converted','won'].includes(v)) return 'ready'; if (isClosed(record)) return 'closed'; return ''; }
  ...
  async function load() {
    ...
    try {
      ...
      const [repairs,leads]=await Promise.all([
        ...
        client.from('leads').select('id,name,phone,email,service,notes,source,status,pipeline_status,device_category,manufacturer,model,customer_issue,contact_attempted_at,created_at,updated_at,intake_method,assigned_user_id,profiles:assigned_user_id(display_name)').order('created_at',{ascending:false}).limit(500)
      ]);
