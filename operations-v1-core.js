(() => {
  ...
  function setStatus(form,message,isError=false){const status=form.querySelector('.operation-status');if(status){status.textContent=message;status.classList.toggle('v1-error',isError);status.classList.toggle('v1-success',!isError);}}
  ...
  async function updateWorkflow(form){
    ...
    if(state.training){const previous=ticket.status;ticket.status=data.status;ticket.updated_at=now();if(data.status==='repaired')ticket.ready_for_pickup_at=now();ticket.ticket_events=ticket.ticket_events||[];ticket.ticket_events.push({id:uid('event'),event_type:data.status!==previous?'status_changed':'note',message:data.note.trim(),created_at:now(),actor:{display_name:state.profile.display_name}});saveTraining();renderAll();openWorkOrder(ticket.id);return;}
    if(data.status!==ticket.status){const result=await client.rpc('advance_repair_status',{target_ticket:ticket.id,next_status:data.status,update_note:data.note.trim(),update_attachments:[],update_visibility:'internal'});if(result.error)return setStatus(form,result.error.message,true);}else{const result=await client.rpc('add_repair_update',{target_ticket:ticket.id,update_note:data.note.trim(),update_attachments:[],update_visibility:'internal'});if(result.error)return setStatus(form,result.error.message,true);}const ticketId=ticket.id;await reload();openWorkOrder(ticketId);
  }
  ...
  function validStep(){const x=state.intake;if(x.step===1)return Boolean(x.newCustomer.first_name&&x.newCustomer.last_name&&x.newCustomer.address_line_1&&x.newCustomer.email&&x.newCustomer.phone&&x.newCustomer.contact_phone);if(x.step===2)return Boolean(x.newDevice.category&&x.newDevice.model&&x.complaint.trim());return true;}
  ...
  async function createFromIntake(){
    ...
    const customerPayload={location_id:state.profile.location_id,first_name:x.newCustomer.first_name.trim(),last_name:x.newCustomer.last_name.trim(),phone:x.newCustomer.phone.trim(),phone_normalized:digits(x.newCustomer.phone),contact_phone:x.newCustomer.contact_phone.trim(),email:x.newCustomer.email.trim().toLowerCase(),address_line_1:x.newCustomer.address_line_1.trim(),address_line_2:x.newCustomer.address_line_2?.trim()||null,city:x.newCustomer.city?.trim()||null,state:x.newCustomer.state?.trim()||null,postal_code:x.newCustomer.postal_code?.trim()||null};
  ...
  function subscribeRealtime(){
    ...
    realtimeChannel=client.channel('portal-v1-operations')
      .on('postgres_changes',{event:'*',schema:'public',table:'leads'},()=>reload())
      .on('postgres_changes',{event:'*',schema:'public',table:'repair_tickets'},()=>reload())
      .on('postgres_changes',{event:'*',schema:'public',table:'purchase_orders'},()=>reload())
      .on('postgres_changes',{event:'*',schema:'public',table:'purchase_order_items'},()=>reload())
      .subscribe();
  }
