(() => {
  ...
  const state = {
    profile:null, permissions:new Map(), defs:[], overrides:[], staff:[], customers:[], workOrders:[], leads:[], inventory:[], services:[], deviceModels:[], guides:[], templates:[], suppliers:[], purchaseOrders:[], poItems:[], intakes:[], appointments:[], settings:null,
  ...
  function automationQa(){return state.profile?.account_type==='automation'||state.profile?.role==='automation';}
  function canPreviewIntake(){return can('repairs.intake')||automationQa();}
  ...
  async function loadProduction(){
    ...
    const jobs={
      ...
      deviceModels:client.from('device_catalog_models').select('id,name,category,image_url,image_source,image_license,image_author,image_attribution_url').eq('active',true),
  ...
  function renderDashboard(){
    ...
    host.innerHTML=`<article class="v1-ops-card"><div class="v1-ops-card-head"><div><h2>Work Orders</h2><p>One searchable table for the shop.</p></div><div class="v1-table-tools"><input id="v1-dash-work-search" aria-label="Search work orders" placeholder="Search work orders"><select id="v1-dash-work-status" aria-label="Filter work orders by status"><option value="active">Active</option><option value="all">All</option><option value="ready">Ready for pickup</option></select></div></div><div class="v1-table-wrap"><table class="v1-table"><thead><tr><th>Work order</th><th>Customer</th><th>Device / issue</th><th>Status</th><th>Tech</th></tr></thead><tbody id="v1-dash-work-body">${workRows(active)}</tbody></table></div></article>${can('leads.view')?`<article class="v1-ops-card"><div class="v1-ops-card-head"><div><h2>Leads</h2><p>Contact, qualify, and move customers toward intake.</p></div><div class="v1-table-tools"><input id="v1-dash-lead-search" aria-label="Search leads" placeholder="Search leads"><select id="v1-dash-lead-status" aria-label="Filter leads by status"><option value="all">All statuses</option>${LEAD_STAGES.map(s=>`<option value="${s}">${leadLabel(s)}</option>`).join('')}</select></div></div><div class="v1-table-wrap"><table class="v1-table"><thead><tr><th>Customer</th><th>Device / issue</th><th>Status</th><th>Last contact</th><th>Created</th></tr></thead><tbody id="v1-dash-lead-body">${leadRows(state.leads)}</tbody></table></div></article>`:''}`;
  ...
  function renderRepairs(){
    ...
    host.innerHTML=`<div class="page-heading"><div><p class="eyebrow">Repair workflow</p><h1>Work Orders</h1><p class="subtle">Search the complete repair pipeline and open the technician workspace.</p></div><div class="v1-actions">${canPreviewIntake()?'<button class="secondary-button" data-v1-walkin>Walk-In</button>':''}<button class="primary-button" data-open-ticket>+ New Work Order</button></div></div><article class="v1-ops-card"><div class="v1-ops-card-head"><div><h2>Work-order table</h2><p>${state.workOrders.length} records</p></div><div class="v1-table-tools"><input id="v1-repair-search" aria-label="Search work orders" placeholder="Search work order, customer, device, phone"><select id="v1-repair-status" aria-label="Filter work orders by status"><option value="active">Active</option><option value="all">All</option>${Object.entries(REPAIR_LABELS).map(([value,label])=>`<option value="${value}">${label}</option>`).join('')}</select></div></div><div class="v1-table-wrap"><table class="v1-table"><thead><tr><th>Work order</th><th>Customer</th><th>Device / issue</th><th>Status</th><th>Tech</th></tr></thead><tbody id="v1-repair-body">${workRows(state.workOrders.filter(t=>!TERMINAL.has(t.status)))}</tbody></table></div></article>`;
  }
  ...
  async function cancelLeadRecord(id){
    const lead=state.leads.find(item=>item.id===id);if(!lead)return;
    const reason=prompt('Why is this lead being closed? This note is kept in its activity history.');if(!reason?.trim())return;
    if(!confirm(`Close ${lead.name||'this lead'} and cancel any linked appointment?`))return;
    if(state.training){
      lead.pipeline_status='lost';lead.status='lost';lead.updated_at=now();lead.last_contact_note=reason.trim();lead._events=lead._events||[];
      lead._events.unshift({id:uid('event'),event_type:'cancelled',message:`Lead closed: ${reason.trim()}`,created_at:now(),profiles:{display_name:state.profile.display_name}});
      const appointment=state.appointments.find(item=>item.id===lead.appointment_id);
      if(appointment&&!['cancelled','completed','no_show'].includes(appointment.status)){appointment.status='cancelled';appointment.notes=`${appointment.notes||''}\nCancelled when lead was closed: ${reason.trim()}`.trim();}
      saveTraining();closeDrawer();renderAll();return;
    }
    const result=await client.rpc('cancel_lead',{target_lead:id,cancellation_note_input:reason.trim()});
    if(result.error){alert(result.error.message);return;}closeDrawer();await reload();
  }
  ...
  function renderReady(){
    ...
    host.innerHTML=`<div class="page-heading"><div><p class="eyebrow">Front counter</p><h1>Ready for Pickup</h1><p class="subtle">Scan the work-order label or choose a completed device from the list.</p></div></div><article class="v1-ops-card"><div class="v1-ops-card-head"><div><h2>Pickup queue</h2><p>${rows.length} device${rows.length===1?'':'s'} waiting</p></div><div class="v1-table-tools"><input id="v1-pickup-scan" aria-label="Scan or enter work-order barcode" autocomplete="off" placeholder="Scan GC work-order barcode"></div></div><div class="v1-table-wrap"><table class="v1-table"><thead><tr><th>Work order</th><th>Customer</th><th>Device</th><th>Ready since</th><th>Balance</th></tr></thead><tbody>${rows.map(t=>`<tr data-v1-work-order="${esc(t.id)}" tabindex="0" role="button" aria-label="Open work order ${esc(ticketCode(t.ticket_number))}"><td><strong>${ticketCode(t.ticket_number)}</strong></td><td><strong>${esc(`${t.customers?.first_name||''} ${t.customers?.last_name||''}`.trim())}</strong><small>${esc(t.customers?.phone||'')}</small></td><td>${esc([t.devices?.manufacturer,t.devices?.model].filter(Boolean).join(' ')||'Device')}</td><td>${new Date(t.ready_for_pickup_at||t.updated_at||t.created_at).toLocaleString()}</td><td>${money(t.total_cents||0)}</td></tr>`).join('')||'<tr><td colspan="5">No devices are ready for pickup.</td></tr>'}</tbody></table></div></article>`;
  }

  function renderReference(){
    if(!can('reference.view'))return;const host=document.getElementById('repair-reference');if(!host)return;
    host.innerHTML=`<div class="page-heading"><div><p class="eyebrow">Internal knowledge</p><h1>Repair Reference</h1><p class="subtle">Staff-only diagnostic and repair reference. Not customer-facing.</p></div></div><div class="v1-reference-search"><input id="v1-guide-search" aria-label="Search repair reference" placeholder="Search PS5 HDMI, battery drain, cracked screen, no power..."></div><div id="v1-guide-grid" class="v1-guide-grid"></div>`;redrawGuides();
  }
  ...
  function deviceFallback(category){const key=String(category||'Other').toLowerCase();return ['phone','tablet','laptop','desktop','console'].includes(key)?`assets/device-${key}.svg`:'assets/gotcracked-mark.png';}
  function deviceImage(device){const model=state.deviceModels.find(item=>item.id===device?.catalog_model_id);return model?.image_url||deviceFallback(device?.category);}
  function bindDeviceImageFallback(host,device){const image=host?.querySelector?.('.v1-device-image img');if(!image)return;image.addEventListener('error',()=>{const fallback=deviceFallback(device?.category);if(image.getAttribute('src')!==fallback)image.setAttribute('src',fallback);},{once:true});}
  ...
  async function cancelWorkOrder(id){
    const ticket=state.workOrders.find(item=>item.id===id);if(!ticket)return;
    if(ticket.payment_status==='paid'||Number(ticket.amount_paid_cents||0)>0){alert('Refund or void the recorded payment before cancelling this work order.');return;}
    const reason=prompt('Why is this work order being cancelled? This note is kept in its activity history.');if(!reason?.trim())return;
    if(!confirm(`Cancel ${ticketCode(ticket.ticket_number)}? Eligible part reservations will be released.`))return;
    if(state.training){
      ticket.status='cancelled';ticket.cancelled_at=now();ticket.cancellation_note=reason.trim();ticket.updated_at=now();ticket.ticket_events=ticket.ticket_events||[];
      ticket.ticket_events.push({id:uid('event'),event_type:'cancelled',message:`Work order cancelled: ${reason.trim()}`,visibility:'internal',created_at:now(),actor:{display_name:state.profile.display_name}});
      saveTraining();renderAll();window.GotCrackedUI?.activateView?.('repairs');return;
    }
    const result=await client.rpc('cancel_work_order',{target_ticket:id,cancellation_note_input:reason.trim()});
    if(result.error){alert(result.error.message);return;}await reload();window.GotCrackedUI?.activateView?.('repairs');
  }
  ...
  function openIntake(prefill={}){
    if(!canPreviewIntake())return;
    state.intake={step:0,search:'',customer:prefill.customer||null,newCustomer:prefill.customer?{...prefill.customer}:{},device:prefill.device||null,newDevice:prefill.device?{...prefill.device}:{category:'Phone'},identifierUnavailable:false,identifierUnavailableReason:'',complaint:prefill.customer_issue||prefill.pendingTicket?.customer_issue||'',visual:{},functional:{},lead:prefill.lead||null,pendingTicket:prefill.pendingTicket||null};
    renderIntake();document.getElementById('v1-intake-dialog').showModal();
  }
  function renderIntake(){const root=document.getElementById('v1-intake-root'),x=state.intake;if(!root||!x)return;const steps=['Customer','Profile','Device','Inspection','Review'];root.innerHTML=`<div class="v1-intake-shell"><div class="v1-intake-head"><div><p class="v1-kicker">Walk-In Intake ${state.training?'· TRAINING':''}</p><h2>${steps[x.step]}</h2></div><button class="icon-button" type="button" data-v1-close-intake aria-label="Close repair intake">×</button></div><div class="v1-stepper">${steps.map((_,i)=>`<span class="v1-step ${i<x.step?'done':i===x.step?'active':''}"></span>`).join('')}</div><div class="v1-intake-body">${intakeStepHTML(x)}</div><div class="v1-intake-actions"><button type="button" class="secondary-button" data-v1-intake-prev ${x.step===0?'disabled':''}>Back</button>${x.step<4?'<button type="button" class="primary-button" data-v1-intake-next>Continue</button>':automationQa()?'<button type="button" class="secondary-button" disabled>QA preview only</button>':'<button type="button" class="primary-button" data-v1-intake-create>Create Work Order</button>'}</div></div>`;bindIntakeInputs();}
  function intakeStepHTML(x){
    ...
    const summary=generateSummary(x);const identity=x.identifierUnavailable?`Identifier unavailable · ${x.identifierUnavailableReason}`:(x.newDevice.serial_number||x.newDevice.imei||'Identifier not required for this device type');return `<div class="v1-review"><div class="v1-review-card"><h4>Customer</h4><p>${esc(x.newCustomer.first_name||'')} ${esc(x.newCustomer.last_name||'')} · ${esc(x.newCustomer.phone||'')} · ${esc(x.newCustomer.email||'')}</p></div><div class="v1-review-card"><h4>Device</h4><p>${esc([x.newDevice.manufacturer,x.newDevice.model].filter(Boolean).join(' ')||x.newDevice.category)} · ${esc(x.newDevice.color||'')} · ${esc(x.newDevice.storage_size||'')} · ${esc(x.newDevice.device_condition||'Condition not recorded')}</p><small>${esc(identity)}</small></div><div class="v1-review-card"><h4>Customer complaint</h4><p>${esc(x.complaint)}</p></div><div class="v1-review-card"><h4>Generated intake summary</h4><p>${esc(summary)}</p></div><p class="v1-muted">The structured checklist and device-identity evidence are saved separately and power repair references and parts sourcing.</p></div>`;
  }
  function bindIntakeInputs(){document.querySelectorAll('[data-intake-customer]').forEach(el=>el.addEventListener('input',()=>state.intake.newCustomer[el.dataset.intakeCustomer]=el.value));document.querySelectorAll('[data-intake-device]').forEach(el=>el.addEventListener('input',()=>state.intake.newDevice[el.dataset.intakeDevice]=el.value));document.querySelector('[data-intake-identifier-unavailable]')?.addEventListener('change',event=>{state.intake.identifierUnavailable=Boolean(event.target.checked);if(state.intake.identifierUnavailable){state.intake.newDevice.serial_number='';state.intake.newDevice.imei='';}else state.intake.identifierUnavailableReason='';renderIntake();});document.querySelector('[data-intake-identifier-reason]')?.addEventListener('change',event=>state.intake.identifierUnavailableReason=event.target.value);document.querySelector('[data-intake-complaint]')?.addEventListener('input',event=>state.intake.complaint=event.target.value);document.querySelectorAll('[data-intake-check]').forEach(el=>el.addEventListener('change',()=>{const [bucket,key]=el.dataset.intakeCheck.split(':');state.intake[bucket][key]=el.value;}));}
  function generateSummary(x){const bad=[],good=[];for(const bucket of ['visual','functional'])for(const [key,value] of Object.entries(x[bucket])){if(['fail','damaged','observed'].includes(value))bad.push(`${friendly(key)}: ${friendly(value)}`);else if(value==='pass')good.push(`${friendly(key)} normal`);}let summary=`Initial intake inspection for ${[x.newDevice.manufacturer,x.newDevice.model].filter(Boolean).join(' ')||x.newDevice.category||'device'}. Customer reports: ${x.complaint||'No complaint recorded'}.`;if(x.identifierUnavailable)summary+=` Device serial/IMEI could not be captured at check-in: ${x.identifierUnavailableReason}.`;if(bad.length)summary+=` Observed concerns: ${bad.join('; ')}.`;if(good.length)summary+=` Functions/conditions observed as normal: ${good.join('; ')}.`;if(!bad.length&&!good.length)summary+=' Functional testing was not completed or findings were not recorded.';return summary;}
  function intakeIdentityEvidence(x){return {identifier_status:x.identifierUnavailable?'unavailable':'captured',unavailable_reason:x.identifierUnavailable?x.identifierUnavailableReason:null,serial_present:Boolean(String(x.newDevice.serial_number||'').trim()),imei_present:Boolean(String(x.newDevice.imei||'').trim()),model_number_present:Boolean(String(x.newDevice.model_number||'').trim()),captured_at:now()};}
  function validStep(){const x=state.intake;if(x.step===1)return Boolean(x.newCustomer.first_name?.trim()&&x.newCustomer.phone?.trim());if(x.step===2){const needsIdentifier=['Phone','Tablet','Laptop','Desktop','Console'].includes(x.newDevice.category||'Phone');if(needsIdentifier&&!x.identifierUnavailable&&!String(x.newDevice.serial_number||'').trim()&&!String(x.newDevice.imei||'').trim())return false;if(x.identifierUnavailable&&!String(x.identifierUnavailableReason||'').trim())return false;}return true;}
  ...
  async function createFromIntake(){
    ...
    if(state.training){
      ...
      const intake={id:uid('intake'),location_id:'training',customer_id:customer.id,device_id:device.id,lead_id:x.lead?.id||null,customer_complaint:x.complaint,visual_findings:{...x.visual,_device_identity:intakeIdentityEvidence(x)},functional_findings:x.functional,generated_summary:summary,completed_by:state.profile.id,completed_at:now(),created_at:now()};
    ...
    const intakeResult=await client.from('intake_sessions').insert({location_id:state.profile.location_id,customer_id:customer.id,device_id:device.id,lead_id:x.lead?.id||null,intake_method:'walk_in',customer_complaint:x.complaint,visual_findings:{...x.visual,_device_identity:intakeIdentityEvidence(x)},functional_findings:x.functional,generated_summary:summary,completed_by:state.profile.id,completed_at:now()}).select().single();if(intakeResult.error)return alert(intakeResult.error.message);
    let ticket=x.pendingTicket;
  ...
  document.addEventListener('keydown',event=>{
    const row=(event.key==='Enter'||event.key===' ')?event.target?.closest?.('[data-v1-work-order],[data-v1-lead]'):null;
    if(row&&event.target===row){event.preventDefault();if(row.dataset.v1WorkOrder)openWorkOrder(row.dataset.v1WorkOrder);else if(row.dataset.v1Lead)openLead(row.dataset.v1Lead);return;}
  ...
  document.addEventListener('click',async event=>{
    ...
    if(target.closest('[data-v1-intake-create]')){if(automationQa())return;if(!validStep())return alert('Complete the required fields first.');await createFromIntake();return;}
