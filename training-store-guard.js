(() => {
  ...
  function renderInventory(){
    ...
    const roleFallback=['owner','manager'].includes(s.profile?.role);
    const inventoryManage=s.permissions?.get?.('inventory.manage') ?? roleFallback;
    const inventoryCount=s.permissions?.get?.('inventory.count') ?? roleFallback;
    const inventoryActions=(inventoryManage||inventoryCount)?`<div class="v1-actions">${inventoryManage?'<button class="primary-button" data-training-add-part>+ Add Training Part</button>':''}${inventoryCount?'<button class="secondary-button" data-training-count>Cycle Count</button>':''}</div>`:'';
    host.innerHTML=`${trainingHeader('Inventory','Sandbox stock, counts, damage, receiving, and labels. No production quantities are touched.',inventoryActions)}<article class="v1-ops-card"><div class="v1-table-wrap"><table class="v1-table"><thead><tr><th>Part</th><th>SKU</th><th>On hand</th><th>Reorder</th><th>Cost</th><th>Price</th><th>Supplier</th><th></th></tr></thead><tbody>${s.inventory.map(item=>`<tr><td><strong>${esc(item.name)}</strong><small>${esc(item.category||'')}</small></td><td>${esc(item.sku||'NO SKU')}</td><td>${item.quantity_on_hand}</td><td>${item.reorder_point||0}</td><td>${money(item.cost_cents)}</td><td>${money(item.sell_price_cents)}</td><td>${esc(item.supplier_name||'—')}</td><td><button class="secondary-button" data-training-label="${item.id}">Label</button>${inventoryManage?` <button class="secondary-button" data-training-adjust="${item.id}">Adjust</button>`:''}</td></tr>`).join('')||'<tr><td colspan="8">No training inventory.</td></tr>'}</tbody></table></div></article>`;
  }
